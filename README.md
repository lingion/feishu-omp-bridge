...
>
> A Feishu/Lark <-> omp bidirectional messaging bridge.
...
| 命令：`/help` `/whoami` `/status` `/reset` `/model` `/sessions` `/resume` `/stop` `/usage` | Hermes 命令集 |
...
| 命令（`/help` `/status` `/model` `/reset` `/whoami` `/stop` `/usage`） | ✅ |
| 多会话持久化 + 重启恢复 | ✅ |
| 访问控制（陌生人不放行） | ✅ |
| 群聊 @bot 检测 + 回复 | ✅ |
| 入群打招呼 | ✅ |
